...
def evaluate(config):
    ...
    def cfg_nnet(x, timesteps, context, label, x_clean):
        ### input text and label condition
        ### 验证不需要输入x_clean, 直接用学习到的prototype
        _cond = nnet_ema(x, timesteps, context=context, y=label, x_clean=x_clean)
        ...
        _uncond = nnet_ema(x, timesteps, context=_empty_context, y=_empty_class, x_clean=x_clean)
   ...
   def get_context_generator():
    ...
    def sample_fn(_n_samples):
        ...
        return dpm_solver_sample(_n_samples, config.sample.sample_steps, context=_context, label=_label, x_clean=None)
